...
socket_active.postgres = function (socket, callback, action) {
    child_process.exec('./shell_scripts/postgres/toggle_active.sh ' + _osname + ' ' + _osversion + ' ' + action, function (err, stdout, stderr) {
        if (err) {
            callback("ERROR: " + err);
        } else {
            callback('success');
...
socket_active.nginx = function (socket, callback, action) {
    child_process.exec('./shell_scripts/nginx/toggle_active.sh ' + _osname + ' ' + _osversion + ' ' + action, function (err, stdout, stderr) {
        if (err) {
            callback("ERROR: " + err);
        } else {
            callback('success');
